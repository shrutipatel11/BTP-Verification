...
tpc_executed = {'src':0}
processes = ['src']
hgc_executed = {'src':0}        #0 if the task is not executed, 1 if it is executed
hgc_results = {'src':''}     #stores the output of a task in the form of a string(argument)
    ...
    while True:
        ...
        if len(processes)!=1 and processes[0]!='src':
            ...
            for i in processes:
                ...
                for j in p:
                    if hgc_executed[j]!=1 and (j not in tpc_results):
                        rmproc.append(i)
                        break
            for i in rmproc:
                processes.remove(i)
        ...
        #for the remaining processes,find the inputs (arguments) from the outputs stored in hgc_results dict
        cmdarg = []
        for j in processes:
            tempstr = j
            tempstr+='.py '
